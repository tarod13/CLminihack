...
class softmax_policy_Net(nn.Module):
    def __init__(self, s_dim, n_actions, noisy=False, lr=3e-4, latent_dim=256): # Originally 256
        super().__init__()
        ...
        self.logits_layer = layer(latent_dim, n_actions)
...
class vision_softmax_policy_Net(softmax_policy_Net):
    ...
    def forward(self, pixels):    
        ...
        logits = self.logit_pipe(features) 
        PA_s = nn.Softmax(dim=1)(logits)
        log_PA_s = nn.LogSoftmax(dim=1)(logits)
        return PA_s, log_PA_s


class multitask_softmax_policy_Net(nn.Module):
    def __init__(self, s_dim, n_actions, n_tasks, lr=3e-4, latent_dim=256):
        super().__init__()
        ...
        self.logits_layer = parallel_Linear(n_tasks, latent_dim, n_actions*n_tasks)
        self.logit_pipe = nn.Sequential(
            parallel_Linear(n_tasks, s_dim, latent_dim),
            nn.ReLU(),
            parallel_Linear(n_tasks, latent_dim, latent_dim),
            nn.ReLU(),
            self.logits_layer            
        )        
    ...
    def forward(self, s):    
        logits = self.logit_pipe(s)
        PA_sT = nn.Softmax(dim=2)(logits)
        log_PA_sT = nn.LogSoftmax(dim=2)(logits)
...
class actor_Net(nn.Module):
    ...
    def llhoods(self, s, a):
        m, std = self(s)
        llhoods = Normal(m, std.abs()).log_prob(a)
        llhoods -= torch.log(1 - a.pow(2) + 1e-6)
        llhoods = llhoods.sum(1, keepdim=True)
        return llhoods
