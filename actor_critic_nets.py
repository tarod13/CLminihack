...
class discrete_vision_actor_critic_Net(nn.Module):
    ...
    def evaluate_critic(self, pixel, next_pixel):
        q = self.q(pixel)
        next_q = self.q_target(next_pixel)
        next_pi, next_log_pi = self.actor(next_pixel)
        log_alpha = self.log_alpha.view(-1,1)
    ...
    def sample_action(self, pixel, explore=True):
        PA_s = self.actor(pixel.unsqueeze(0))[0].squeeze(0).view(-1)
